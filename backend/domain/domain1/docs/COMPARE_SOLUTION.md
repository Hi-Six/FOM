...
| 경량 JSON 로드 | `comparison_service.py` | `load_comparison_fields()` 추출 |
| frame_diffs 제한 | `accuracy_scorer.py` | `detail_level` 파라미터 추가, 기본 summary |
| 중복 매칭 경고 | `alignment.py` + `comparison_service.py` | `duplicate_ratio` 계산 및 응답 포함 |

### Phase 2 — 단기 (2~3일)

| 항목 | 파일 | 변경 내용 |
|------|------|----------|
| offset 파라미터 | `compare_request.py`, `alignment.py` | `user_offset_sec`, `ref_offset_sec` 추가 |
| DTW 정렬 | `alignment.py`, `comparison_service.py` | `fastdtw` 기반 구현 |
| 비선형 각도 유사도 | `accuracy_scorer.py` | `_angle_similarity` 함수 개선 |
| detail_level 파라미터 | `compare_request.py` | `summary`/`full` 선택 |
