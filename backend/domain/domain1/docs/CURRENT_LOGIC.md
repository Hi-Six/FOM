...
[비교 단계] POST /video/compare
  ① 저장된 JSON 2개 로드
  ② align_by_time (time_sec 기반 프레임 매칭)
  ③ score_accuracy
     - joint_angles 차이 → 60%
     - bone_vectors 코사인 → 40%
  → Accuracy 점수 + frame_diffs
```
...
### 2.4 시간·템포 차이 (Temporal Alignment) ⚠️

**문제:** 사용자가 늦게 시작, 중간에 빠르게/느리게 → 프레임 어긋남.

**구현 (`alignment.py` Line 6–30):**
```python
def align_by_time(user_frames, ref_frames):
    # 사용자 각 프레임의 time_sec에 가장 가까운 전문가 프레임 매칭
    for uf in user_frames:
        best_rf = min(ref_frames, key=lambda rf: abs(rf.time_sec - uf.time_sec))
```

**효과:**
- 같은 `time_sec`끼리 매칭 → FPS 차이는 어느 정도 흡수
- 구현 **빠름** (O(n×m)), 단순

**한계:**
- **영상 0초 = 춤 시작 시점** 가정 → 오프셋 보정 없음
- 중간에 **템포 변화** (멈춤·빠른 구간) → 매칭 어긋남
- **DTW (Dynamic Time Warping)** 미구현 → `COMPARISON_STRATEGY.md` §3.2 옵션 2 보류

**확장 여부:** Phase 2D (문서 §6.4) — Accuracy 오차가 "시간 동기화"로 재현되면 DTW 검토.
...
**주의:** `total_score` = `accuracy.score` (현재 Accuracy만 구현).
...
| **시간·템포** | ⚠️ 약함 | `time` 정렬만, DTW 없음 → 오프셋·템포 차이 민감 |
| **3D 회전** | ❌ 없음 | Z축 노이즈 → 각도·벡터 오차 가능 |
| **촬영 통제** | ❌ 없음 | 프론트 가이드 UI 미연동 |
| **6개 채점** | 🔄 Accuracy만 | ROM, Power 등 미구현 |
...
**DTW 추가 시:** +60~180초 (fastdtw 없을 경우 O(n²)).
...
**Q3. 사용자가 늦게 시작하면?**  
A. `align_by_time`은 **영상 0초 기준** → 오프셋 미보정. 가능한 **같은 시점 시작** 또는 DTW 대기.
...
**한 줄 요약:** 화면 위치·체형은 정규화 + 각도·벡터 비교로 상당히 완화했고, 시점은 부분 대응, 시간 정렬·3D 회전은 약하거나 없으며, Accuracy(일치도)만 동작하는 MVP 상태입니다.
